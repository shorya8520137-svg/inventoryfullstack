...
**Frontend API Call:**
```javascript
const response = await fetch(`https://api.hunyhuny.org.nip.io/api/timeline/${productCode}`);
...
1. **Get timeline for XYZ789:**
   ```
   curl "https://api.hunyhuny.org.nip.io/api/timeline/XYZ789"
   ```

2. **Get timeline with filters:**
   ```
   curl "https://api.hunyhuny.org.nip.io/api/timeline/XYZ789?warehouse=BLR_WH&dateFrom=2025-01-01"
   ```

3. **Get timeline summary:**
   ```
   curl "https://api.hunyhuny.org.nip.io/api/timeline?warehouse=BLR_WH"
   ```